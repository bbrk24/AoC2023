...
public class Main {
    ...
    private static final long EMPTY_SPACE_SIZE = 1000000L;
    ...
    public static long f(String arg) {
        ...
        for (Point p1 : galaxyLocations) {
            for (Point p2 : galaxyLocations) {
                long length = Math.abs(p1.x - p2.x) + Math.abs(p1.y - p2.y);
                length += (EMPTY_SPACE_SIZE - 1L) * (long)countInRange(p1.x, p2.x, emptyColumnsArr);
                length += (EMPTY_SPACE_SIZE - 1L) * (long)countInRange(p1.y, p2.y, emptyLinesArr);
                total += length;
            }
        }
